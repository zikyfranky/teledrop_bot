...
class UserCompleted(Resource):
    def get(self, user_id):
        isComplete = get_isCompleted(user_id)
        return isComplete

    def put(self, user_id):
        v = request.form['value']
        return update_isCompleted(user_id, v)
...
api.add_resource(UserTwitterLink, '/<string:user_id>/twitter-retweet-link')
...
api.add_resource(UserCompleted, '/<string:user_id>/completed')
